...
import pandas as pd
import numpy as np
import datetime  # تم التعديل لضمان استخدام datetime و timedelta بشكل صحيح
...
# التحديث البرمجي السليم لقراءة الروابط (Query Params) في نسخ Streamlit الحديثة
...
# --- دالة التلوين المحدثة والمتوافقة بالكامل مع Pandas الجديدة ---
...
if results:
    ...
    try:
        # هنا تم التأمين المباشر للتلوين ليعمل على كل بيئات عمل الباندا الحديثة والقديمة
        styled_df = df_results.style.apply(style_table_rows, axis=None).set_properties(**{'text-align': 'center'})
        st.dataframe(styled_df, use_container_width=True)
    except Exception as style_error:
        # في حال حدوث أي مشكلة في التلوين، يتم عرض الجدول الافتراضي الصافي فوراً لمنع تعطل اللوحة
...
saudi_time = datetime.datetime.utcnow() + datetime.timedelta(hours=3) # تحويل دقيق وتجنب تداخل التسميات
